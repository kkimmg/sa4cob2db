...
public class ACMSQLNetSession extends ACMSQLSession implements Runnable {
  ...
  protected void commitTransaction() throws IOException {
    try {
      ...
      writeLine(new FileStatus(FileStatus.STATUS_99_FAILURE, e.getSQLState(), e.getErrorCode(),
          e.getMessage()));
    }
  }
  ...
  protected void open() throws IOException {
    ...
    if (file != null) {
      ...
      if (mode == CobolFile.MODE_INPUT || mode == CobolFile.MODE_OUTPUT
          || mode == CobolFile.MODE_EXTEND || mode == CobolFile.MODE_INPUT_OUTPUT) {
        writeLine(FileStatus.READY);
      } else {
        writeLine(new FileStatus(FileStatus.STATUS_PERMISSION_DENIED, FileStatus.NULL_CODE, 0,
            "can't open file"));
        return;
      }
      // //////////////////////////////////////////////////////
      ...
      if (accessmode == CobolFile.ACCESS_SEQUENTIAL || accessmode == CobolFile.ACCESS_DYNAMIC
          || accessmode == CobolFile.ACCESS_RANDOM) {
  ...
  protected void processMethod(String method) throws IOException {
    ...
    if (method.equals("ASSIGN")) {
      ...
      writeLine(new FileStatus(FileStatus.STATUS_98_UNSUPPORTED_METHOD, FileStatus.NULL_CODE, 0,
  ...
  protected void read() throws IOException {
    ...
    if (file != null) {
      ...
      if (recordBytes.containsKey(file)) {
        ...
        int recordLength = (file.getMetaData() == null ? getMaxLength() : file.getMetaData()
            .getRowSize());
        ...
        try {
          ...
          if (method.equals(ACMNetSession.MSG_INITIALIZE)) {
            ...
            if (isInitialized()) {
              ...
              writeLine(new FileStatus(FileStatus.STATUS_99_FAILURE, FileStatus.NULL_CODE, 0,
                  "not initialized."));
              terminate();
            }
          }
