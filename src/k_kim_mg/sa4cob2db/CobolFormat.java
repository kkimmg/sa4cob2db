...
public class CobolFormat extends DecimalFormat {
  ...
  static class PatternFlags {
     ...
     *          #
     */
    public PatternFlags(CobolColumn column, boolean db, boolean cr, boolean plus, boolean minus,
        boolean sla, boolean bs, boolean ast, boolean srp) {
      this.column = column;
      this.db = db;
      this.cr = cr;
      this.plus = plus;
  ...
  public static NumberFormat createFormatter(CobolColumn column) {
    ...
    if (l_db | l_cr | l_plus | l_minus | l_sla | l_bs | l_ast | l_z) {
      PatternFlags flags = new PatternFlags(column, l_db, l_cr, l_plus, l_minus, l_sla, l_bs,
          l_ast, l_z);
