...
public abstract class AbstractCobolFile implements CobolFile {
	...
	protected class DefaultSequentialReadBuffer implements Runnable, SequentialReadBuffer {
		...
		synchronized void addCs(int i) {
			setCs(getCs() + i);
		}

		synchronized void addRb(int i) {
			setRb(i);
		}

		synchronized void addRi(int i) {
			setRi(i);
		}

		synchronized void addWb(int i) {
			setWb(i);
		}

		synchronized void addWi(int i) {
			setWi(i);
		}

		synchronized int getCs() {
			return cs;
		}

		synchronized int getRb() {
			return rb;
		}

		synchronized int getRi() {
			return ri;
		}

		synchronized int getWb() {
			return wb;
		}

		synchronized int getWi() {
			return wi;
		}
		...
		boolean isWaitToRead() {
			boolean ret = false;
			if (!lastRead) {
				if (!initCleared && getCs() < initSize) {
					ret = true;
				}
				if (!buffStatus[getRb()][getRi()]) {
					ret = true;
				}
				if (getCs() < 0) {
					ret = true;
		...
		boolean isWaitToWrite() {
			boolean ret = false;
			ret = (getCs() >= maxSize);
			ret = (ret || buffStatus[getWb()][getWi()]);
			return ret;
		}
		...
		public synchronized FileStatus nextBuffer() {
			...
			//
			FileStatus ret = nextStatus[getRb()][getRi()];
			if (ret != null) {
				if (!ret.getStatusCode().equals(FileStatus.STATUS_SUCCESS)) {
					if (lastRead) {
						nextStatus[getRb()][getRi()] = STATUS_EOF;
					} else {
						buffStatus[getRb()][getRi()] = false;
					}
				}
			}
			return ret;
		...
		public synchronized FileStatus readBuffer(byte[] record) {
			addCs(-1);
			if (getCs() <= minSize && initCleared && !lastRead) {
				initCleared = false;
			}
			System.arraycopy(records[getRb()][getRi()], 0, record, 0, (records[getRb()][getRi()].length > record.length ? record.length : records[getRb()][getRi()].length));
			FileStatus ret = readStatus[getRb()][getRi()];
			if (lastRead) {
				readStatus[getRb()][getRi()] = STATUS_EOF;
			}
			buffStatus[getRb()][getRi()] = false;
			return ret;
		}
		...
		public void run() {
			...
			while (cont) {
				...
				if (cont) {
					//
					ns = nextOnFile();
					nextStatus[getWb()][getWi()] = ns;
					if (ns.getStatusCode().equals(FileStatus.STATUS_SUCCESS)) {
						rs = readFromFile(records[getWb()][getWi()]);
						readStatus[getWb()][getWi()] = rs;
						buffStatus[getWb()][getWi()] = true;
						//
						addCs(1);
						addWi(1);
						if (getWi() >= dimentionSize) {
							setWb((getWb() == 0 ? 1 : 0));
							setWi(0);
						}
						if (!initCleared && getCs() >= initSize) {
		...
		synchronized void setCs(int cs) {
			this.cs = cs;
		}

		synchronized void setRb(int rb) {
			this.rb = rb;
		}

		synchronized void setRi(int ri) {
			this.ri = ri;
		}

		synchronized void setWb(int wb) {
			this.wb = wb;
		}

		synchronized void setWi(int wi) {
			this.wi = wi;
		}
