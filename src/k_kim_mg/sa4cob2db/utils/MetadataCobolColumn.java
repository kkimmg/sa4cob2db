...
public class MetadataCobolColumn extends ArrayList<MetadataCobolColumn> {
   ...
   *          document
   * @param parent
   *          node that indicates parent or record.
   * @param start
   *          start index
   * @param fix
   *          fix string
  ...
  protected int exportToNode1(Document document, Node parent, int start, String fix) {
    ...
    if (l_length > 0) {
      ...
      map.setNamedItem(setNodeAttribute(document, "originalColumnName", getOriginalColumnName()
          + fix));
  ...
  void parceUsage(String token) {
    ...
    if (l_usage.equals("BINARY") || l_usage.equals("BINARY.") || l_usage.equals("COMP")
        || l_usage.equals("COMP.") || l_usage.equals("COMPUTATIONAL")
        || l_usage.equals("COMPUTATIONAL.")) {
      usage = CobolColumn.USAGE_BINARY;
    } else if (l_usage.equals("PACKED-DECIMAL") || l_usage.equals("PACKED-DECIMAL.")
        || l_usage.equals("COMP-3") || l_usage.equals("COMP-3.")
        || l_usage.equals("COMPUTATIONAL-3") || l_usage.equals("COMPUTATIONAL-3.")) {
