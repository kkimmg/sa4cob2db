...
static struct option longopts[] = {
		{ "metafile", required_argument, NULL, 'm' }, { "linein",
				required_argument, NULL, 'l' }, { "extend", required_argument,
...
extern int main_too (int argc, char *argv[]) {
	...
	char* extend = "false";
	char* commit = "0";
	while ((opt = getopt_long(argc, argv, "m:l:e:c:h", longopts, NULL)) != -1) {
		switch (opt) {
		case 'm':
			metafile = optarg;
			break;
	...
	if (argc > optind + 1) {
		...
		jstring s_commit = (*env)->NewStringUTF(env, commit);
		jstring s_display_usage = (*env)->NewStringUTF(env, "false");
		// execute
		(*env)->CallStaticVoidMethod(env, clazz, midMainToo, s_acmfile,
				s_infile, s_metafile, s_linein, s_extend, s_commit, s_display_usage);
	} else {
		display_usage();
	}
...
extern int initializeJNI() {
	...
	// get method
	midMainToo =
			(*env)->GetStaticMethodID(env, clazz, "main_too",
