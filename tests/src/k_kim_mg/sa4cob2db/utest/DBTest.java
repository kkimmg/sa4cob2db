...
public class DBTest {
	...
	public void testReadSeq1() {
		...
		assertEquals("Open Status", FileStatus.OK.getStatusCode(), status.getStatusCode());
		status = file.read(bytes);
		assertEquals("Read Status", FileStatus.OK.getStatusCode(), status.getStatusCode());
		status = file.close();
		assertEquals("Close Status", FileStatus.OK.getStatusCode(), status.getStatusCode());
	}
	...
	public void testReadKey1() throws Exception {
		...
		assertEquals("Move Status", FileStatus.OK.getStatusCode(), status.getStatusCode());
		status = file.read(bytes);
		assertEquals("Read Status", FileStatus.OK.getStatusCode(), status.getStatusCode());
		status = file.close();
		assertEquals("Close Status", FileStatus.OK.getStatusCode(), status.getStatusCode());
